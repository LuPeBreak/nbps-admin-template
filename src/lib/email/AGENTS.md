...
- **Delivery Limitation**: Validate recipient syntax before calling the mailer. A successful `sendEmail` result means the transport accepted the message; it does not prove that the mailbox exists or that delivery succeeded. Monitor asynchronous bounces through the email provider when required.
