...
- **Capability Boundary**: Choose a catalog-derived `PermissionRequest` (AND) or `{ anyOf: PermissionRequest[] }` (OR of complete AND alternatives) in server code. Under the current NBPS baseline, `protectedAction` validates the session, single configured role and capability requirement even on direct invocation; the callback receives the validated session and original arguments. Empty/malformed requirements deny access. Role-model evolution is governed by `src/lib/auth/AGENTS.md`.
- **Contextual Authorization**: The callback owns domain restrictions after the capability gate. A hypothetical own/any rule is `canUpdateAny || (canUpdateOwn && targetOwnerId === session.user.id)`. Obtain target ownership from trusted server data and enforce it before restricted reads or mutations; a client-supplied owner ID is not proof. OR alone never checks ownership, and the generic helper must not resolve owners. NBPS does not currently define own permissions.
