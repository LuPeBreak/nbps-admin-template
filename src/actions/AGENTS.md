...
- **Capability Boundary**: Choose a catalog-derived `PermissionRequest` (AND) or `{ anyOf: PermissionRequest[] }` (OR of complete AND alternatives) in server code. `protectedAction` validates the session, single configured role and capability requirement even on direct invocation; the callback receives the validated session and original arguments. Empty/malformed requirements deny access.
