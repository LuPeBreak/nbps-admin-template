...
- **Client-Side Permission Checks**: Action buttons and row actions use synchronous `authClient.admin.checkRolePermission` with a simple catalog-derived AND request. Compose OR only when needed with separate native calls per alternative; `anyOf` is not a native client payload. Hide/disable unauthorized controls and hide the entire dropdown cell if all actions are restricted. These checks are UX only; pages, custom actions and native Better Auth endpoints keep server authority. `Novo Usuário` requires `user: ["create", "set-role"]` because its form submits a role; row actions retain individual capability checks.
- **Hydration-Stable Permission Inputs**: When a protected Server Component already has the validated session, pass the validated role (a single role in the current NBPS baseline) and current user ID to SSR-rendered Client Components as serializable props. Use those stable inputs for the initial permission-gated tree; do not make the first render depend on an asynchronously hydrated `useSession()` result. Role-model evolution is governed by `src/lib/auth/AGENTS.md`.
