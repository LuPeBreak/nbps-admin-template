...
Para executar a suíte mínima de testes unitários com Vitest:
...
Esta suíte inicial cobre regras puras de permissões, validação de schemas Zod e helpers utilitários, não fluxos de integração ou interface. O GitHub Actions executa automaticamente a suíte de testes unitários, lint, typecheck e build em todo push ou pull request para as branches `main` e `development`.
...
*   **Ações e Leituras de Banco:** No runtime da aplicação, nunca execute `prisma.*` em páginas, layouts ou componentes. Leituras e escritas da aplicação passam por Server Actions (`.action.ts`). Scripts server-only de infraestrutura, como seeds, migrations e configuração de adapters, são exceções explícitas.
...
| **App Routing** | `src/app/AGENTS.md` | Estrutura de layouts, páginas e orquestração de Server Components. |
...
| **Erros** | `src/lib/errors/AGENTS.md` | Contrato de erro unificado e taxonomia de respostas. |
| **Action Helpers** | `src/lib/actions/AGENTS.md` | Utilitários `validateInput` e `actionError`. |
| **Validações** | `src/validations/AGENTS.md` | Schemas Zod como fonte única da verdade e tipos derivados. |
| **Server Actions** | `src/actions/AGENTS.md` | Implementação de Server Actions e regras de validação. |
| **Componentes** | `src/components/AGENTS.md` | Primitivos UI, hooks (`useDialogAction`) e estilização. |
| ↳ **Sidebar** | `src/components/sidebar/AGENTS.md` | Menu lateral com controle visual de permissões. |
| ↳ **Data Table** | `src/components/data-table/AGENTS.md` | Tabelas dinâmicas server-side integradas com `nuqs`. |
| **Proxy** | `src/proxy.ts` | Rotas protegidas usando proxy Edge nativo Next.js 16. |
